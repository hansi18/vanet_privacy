...
Implements the Cloud Layer (Global Digital Twin):
  ...
  - ε policy envelope update per role tier
  - Attack simulation / adversary stress test feedback
"""
...
class CloudDT:
    def __init__(self, model_template=None):
        ...
        # Anomaly detection sensitivity (updated by attack simulation)
        self.norm_sigma_mult = 2.0
        self.cosine_floor = 0.70
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Attack simulation feedback
    # ─────────────────────────────────────────────────────────────────────────

    def attack_simulation_update(self, flagged_fraction):
        """
        If high fraction flagged → tighten screening thresholds pushed to RSUs.
        """
        if flagged_fraction > 0.15:
            self.norm_sigma_mult = max(1.5, self.norm_sigma_mult - 0.1)
            self.cosine_floor = min(0.85, self.cosine_floor + 0.02)
        else:
            self.norm_sigma_mult = min(2.5, self.norm_sigma_mult + 0.05)
            self.cosine_floor = max(0.65, self.cosine_floor - 0.01)

        return {
            "norm_sigma_mult": self.norm_sigma_mult,
