...
class RSUDT:
    ...
    def __init__(self, eps_min=0.05, eps_max=1.50, clip_C=1.0,
                 norm_sigma_mult=2.0, cosine_floor=0.70, min_history=5):
        self.twins = {}
        self.eps_min = eps_min
        self.eps_max = eps_max
        self.clip_C = clip_C
    ...
    def screen_gradient(self, twin, gradient, tier_tag, eps_consumed):
        ...
        Multi-signal anomaly detection:
          1. Norm check   — ||g|| vs (μ ± 2σ) from twin history
          2. Cosine check — cosine similarity < 0.70 → slow-drift flag
          3. Tier tag     — sudden sensitivity spike
          4. Budget check — over/under consumption
    ...
    def update_trust(self, twin, flags):
        ...
        if not flags:
            ...
            # Faster trust loss — proportional to number of signals
            penalty = 0.10 * len(flags)
            twin.trust_score = max(0.0, twin.trust_score - penalty)
            twin.consecutive_flags += 1

        # RBAC consequence table (from slides)
    ...
    def aggregate(self, updates):
        """
        Weighted FedAvg:
          weight = trust_score × |D_i| × flag_penalty
          flag_penalty: 1.0 (clean), 0.30 (one flag), 0.10 (2+ flags)
        ...
        for u in updates:
            ...
            # Extra penalty for low-trust vehicles
            if twin.trust_score < 0.10:
                penalty = 0.05
